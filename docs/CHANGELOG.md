...
Two design questions raised in the issue thread were resolved as follows:
- **Profiles vs. separate compose files** — one `docker-compose.yml`, not a second file: added `print-farm-manager-dev` behind Compose's `profiles: ["dev"]` so a bare `docker compose up` never starts it.
- **A `dev` Dockerfile stage**, not a second Dockerfile — split the existing `server-deps` stage into `deps` (full `npm ci`, unpruned) and `server-deps` (extends `deps`, adds the prune step — used by `runtime` exactly as before), then added a `dev` stage extending `deps` so it reuses the apt-get/native-toolchain layer instead of repeating it.

Three problems surfaced only by actually running the containers end-to-end (not just building them), all fixed before landing:
- `server/index.js`'s `client/dist/index.html` existence check isn't scoped to `NODE_ENV`, so a fresh clone with no prior `npm run build` crashes the server side of `npm run dev` inside the container exactly as it does natively (the root cause behind issue #15). Rather than touch that check — which would step on the separate fix already being proposed against that issue — the `dev` stage's `CMD` now builds the client once on first start if `client/dist/index.html` is missing, then runs `npm run dev` as normal. Verified: a container started with `client/dist` absent auto-builds once, then serves normally; a subsequent restart with `client/dist` present (persisted via the bind mount) skips straight to `npm run dev`.
- Vite's dev server binds `127.0.0.1` by default, which is unreachable through Docker's port publishing — `:5173` would appear to be running (in logs) but never actually answer from the host. Fixed via a `VITE_DOCKER_HOST` env var read only by `client/vite.config.js` and set only by the `dev` compose service, so native `npm run dev` keeps its existing localhost-only behavior. Verified via `curl` against both `:3000` (API) and `:5173` (Vite, including the `/api` proxy through it).
- `docker compose --profile dev up` (no service name) starts the `dev`-profiled service *alongside* the default `print-farm-manager` service rather than instead of it — Compose profiles are additive, not exclusive — and both publish host port 3000, so the second one to bind fails with "address already in use." Fixed by documenting (and testing) the service-name form instead: `docker compose up --build print-farm-manager-dev`. Naming a service explicitly starts only that service and its dependencies, bypassing both the profile gate and the "also start every default service" behavior a bare `up` has — verified in both directions: the named-service form starts only `print-farm-manager-dev` (production untouched), and a plain `docker compose up` starts only `print-farm-manager` (dev untouched).

Also verified: hot reload actually round-trips through the bind mount (`docker compose up --build print-farm-manager-dev` running, edited `client/src/App.jsx` on the host, saw `[vite] (client) hmr update /src/App.jsx` in the container's logs), and `docker compose exec print-farm-manager-dev npm test` runs the full suite inside the container — a side benefit, since the `dev` stage carries the same devDependencies and native build tools this project's own `CONTRIBUTING.md` already recommends running tests under (Node 22, not whatever the host has).

### Changes
- `Dockerfile`: split `server-deps` into `deps` (unpruned, shared) + `server-deps` (pruned, used by `runtime`, behavior unchanged); added a `dev` stage (`deps` + `client/node_modules`) whose `CMD` builds `client/dist` on first run if missing (`&&`, not `;` — a failed build stops the container instead of falling through), then runs `npm run dev`.
- `docker-compose.yml`: pinned the existing `print-farm-manager` service to `target: runtime` explicitly (previously implicit as "last stage" — no longer true now that `dev` is defined after it); added `print-farm-manager-dev` behind `profiles: ["dev"]`, bind-mounting the repo with `node_modules`/`client/node_modules` shadowed by anonymous volumes (avoids bind-mounting a host `node_modules` built for the wrong platform/ABI — see `better-sqlite3` note in `CONTRIBUTING.md`), publishing `3000`+`5173`, and using separate `farm-data-dev`/`farm-gcode-dev` named volumes so dev data can't collide with a production instance on the same host.
- `client/vite.config.js`: `server.host` now reads `VITE_DOCKER_HOST` (set only in the `dev` compose service) to bind `0.0.0.0` instead of the default localhost-only, so `:5173` is reachable from the host through Docker's port mapping.
- `.github/workflows/docker-publish.yml`: both `docker/build-push-action` calls (`build` and `pr_test_build` jobs) now pass `target: runtime` explicitly — same "last stage" default that `docker-compose.yml` needed pinning against also applied here, and without it either job would have published/validated the `dev` stage instead of production once it was added after `runtime`.
